...
$stmt = $pdo->query("SELECT id, username FROM users WHERE role = 'player'");
...
// Skala i marginesy
$margin = 40;
$barSpacing = ($width - 2 * $margin) / (count($values) - 1);
...
foreach ($values as $i => $val) {
    $x = $margin + $i * $barSpacing;
    $y = $height - $margin - $val * $scale;
    $points[] = [$x, $y];
...
foreach ($labels as $i => $label) {
    $x = $margin + $i * $barSpacing - 10;
    $y = $height - $margin + 10;
    imagestring($image, 4, $x, $y, $label, $black);
}

// Wygeneruj dane binarne obrazu PNG i przekaż do <img>
$graphData = ob_get_clean();
ob_start();
imagepng($image);
